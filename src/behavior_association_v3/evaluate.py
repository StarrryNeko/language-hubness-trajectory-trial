...
def main():
    ...
    blockers = []
    allowed_finish_reasons = {"native_eos", "text_boundary", "token_ceiling"}
    ...
    if any(int(row.get("generated_token_count", -1)) < protocol["decoding"]["max_new_tokens"] for row in ceiling_rows):
    ...
    write_manifest(paths(cfg).metrics / f"{args.split}_evaluation_manifest.json", {
        ...
        "observed": observed, "gates": gates, "status": status, "blockers": blockers,
