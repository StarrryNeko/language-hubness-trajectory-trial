...
在完全相同语义的平行译句集合中，英语 mean-pool 表示是否比其他语言更频繁地成为多语言表示图的中心或 reverse-kNN hub？这一现象是否跨层、跨来源语言群体、跨局部密度控制和跨模型家族稳定？
...
- 只提取 `mean_pool`，不计算任何其他句向量表示；
- 主条件与 local-scaled cosine 四指标在相同连续层成立；
- k=1/3/5/10 的结论一致；
...
2. 查看 `validation/validation_summary.md`，确认无截断、只存在 mean-pool、候选范围严格同语义。
